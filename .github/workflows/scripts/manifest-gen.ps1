...
# Ensures we find 'channel_id' regardless of casing or format
...
function Resolve-TargetChannelID($MessageList) {
    ...
    foreach ($m in $MessageList) {
        ...
        if ($null -ne $ThreadParent -and $ThreadParent -eq "1") {
            ...
            if ($EnableDebugMode) { Write-Host "[DEBUG] ID: Priority 1 (Parent 1) matched: $ResID" -ForegroundColor Gray }
            return $ResID
        }
    }
    ...
    foreach ($m in $MessageList) {
        ...
        if ($null -ne $ThreadParent -and $ThreadParent.Length -gt 10 -and $ThreadParent -ne "1") {
            if ($EnableDebugMode) { Write-Host "[DEBUG] ID: Priority 2 (Snowflake Parent) matched: $ThreadParent" -ForegroundColor Gray }
            return $ThreadParent
        }
    }
    ...
    if ($null -ne $IdTally) {
        ...
        if ($EnableDebugMode) { Write-Host "[DEBUG] ID: Priority 3 (Majority Rule) matched: $Winner" -ForegroundColor Gray }
        return $Winner
    }
...
foreach ($RemoteFile in $ValidFiles) {
    ...
    try {
        ...
        $MainPosts = $Messages | Where-Object { 
            ...
            # EXCLUSION: Only count toward main if it matches the resolved ID OR has no thread metadata
        ...
        # Thread Grouping
        # FIX: Exclude the ResolvedID itself to prevent Preludes from listing themselves as threads
        ...
        if ($null -eq $Record) {
            ...
            if ($null -eq $Record.order) { 
                $CurrMax = if ($TargetCampaign.logs.Count -gt 0) { ($TargetCampaign.logs | Measure-Object -Property order -Maximum).Maximum } else { -1 }
                $Record.order = $CurrMax + 1 
            }
        ...
        foreach ($G in $ThreadGroups) {
            ...
            if ($null -ne $StoredT) {
                ...
                $RefreshedThreads.Add($StoredT)
            } else {
                $RefreshedThreads.Add([PSCustomObject]@{ threadID = $LookupTid; displayName = $G.Group[0].thread.name; isActive = $true; isNSFW = $false; messageCount = $TCount })
            }
        }
        $Record.threads = $RefreshedThreads.ToArray()
